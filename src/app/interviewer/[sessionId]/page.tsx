...
function analyzeTranscript(transcript: TranscriptEntry[]) {
  ...
  const avgAgentLength =
    agentMessages.reduce((sum, m) => sum + m.content.length, 0) /
    Math.max(agentMessages.length, 1);
  ...
  dimensions.push({
    label: "Turn-taking",
    score: Math.round(turnTakingScore),
    max: 100,
  });
...
function buildGateData(scores: any[], events?: any[]) {
  ...
  if (latest.overall_score !== undefined || latest.dimension_scores) {
    ...
    const dimensions = Object.entries(dimensionScores).map(
      ([label, score]) => ({
        label,
    ...
    const followups = normalizeFollowups(
      latest.recommended_followups || latest.followups,
    );
    const derived =
      followups.length === 0
  ...
  const confidence =
    dimensionRows.reduce(
      (acc: number, row: any) => acc + Number(row.confidence || 0),
      0,
    ) / Math.max(1, dimensionRows.length);
...
function normalizeEvidenceQuotes(evidence: any) {
  ...
  if (Array.isArray(evidence)) {
    return evidence
      .map((item) => ({
        dimension: String(item.dimension || item.label || "Evidence").replace(
          /_/g,
          " ",
        ),
...
function InterviewerView() {
  const { session, scopePackage, scores, events, rounds, artifacts, refresh } =
    useSession();
  ...
  const [widgetConfigState, setWidgetConfigState] = useState<
    "idle" | "saving" | "saved" | "error"
  >("idle");
  const [widgetConfigError, setWidgetConfigError] = useState<string | null>(
    null,
  );
  ...
  const [resumePreviewState, setResumePreviewState] = useState<
  ...
  const [flagSeverity, setFlagSeverity] = useState<"warning" | "critical">(
    "warning",
  );

  const voiceActiveRound = (rounds || []).find(
    (round) => round.status === "active",
  );
  const isVoiceRealtimeActive =
    voiceActiveRound?.round_type === "voice-realtime";
  ...
  const resumeUrl = useMemo(
    () => resolveResumeUrl(artifacts as any[]),
    [artifacts],
  );
  ...
  useEffect(() => {
    const configured = (scopePackage as any)?.simulation_payloads
      ?.role_widget_config;
    const roleFamily = configured?.role_family || roleTrack;
    const lanes = normalizeRoleWidgetConfig(configured?.lanes);
    const resolved =
      lanes.length > 0 ? lanes : getRoleWidgetTemplate(roleFamily);
  ...
  const completedRounds = roundTimeline.filter(
  ...
  const resumeSkills = Array.isArray(candidateInsights.resume_skills)
    ...
    : [];
  const insightLine = [
    candidateInsights.interview_level
      ? `Interview level ${candidateInsights.interview_level}`
      : null,
  ...
  const sendAction = async (
    ...
    if (!response.ok) {
      setActionNotice({
        kind: "error",
        message: data?.error || `Action failed: ${action_type}`,
      });
    ...
    setActionNotice({
      kind: "success",
      message: `Action applied: ${action_type.replace(/_/g, " ")}`,
    });
    await refresh();
    return { ok: true, data };
  };
  ...
  const sendCandidateLink = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data.error || "Failed to generate candidate access link.",
        );
      }
  ...
  const setSessionStatus = async (
    ...
    if (!response.ok) {
      setActionNotice({
        kind: "error",
        message: data?.error || "Failed to update session status.",
      });
      return;
    }
        ...
        <header className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Button
              ...
              variant="outline"
              onClick={() =>
                void setSessionStatus("completed", "interviewer_complete")
              }
            >
              Mark Completed
            </Button>
            <Button
              ...
              variant="destructive"
              onClick={() =>
                void setSessionStatus("aborted", "interviewer_abort")
              }
            >
              Abort Session
            </Button>
        ...
        <div className="grid w-full gap-6 lg:grid-cols-[1.15fr,1fr]">
          <section className="space-y-6">
            <Card>
              <CardHeader className="space-y-3">
                ...
                <CardDescription>
                  ...
                  {candidateEmail || "No candidate email on file"}
                </CardDescription>
                {insightLine && (
              ...
              <CardContent className="space-y-3">
                <div className="flex flex-wrap gap-2">
                  ...
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => sendAction("inject_curveball")}
                  >
                  ...
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => sendAction("switch_persona")}
                  >
                ...
                {sendLinkState === "error" && (
                  <p className="text-sm text-destructive">
                    {sendLinkError || "Unable to generate candidate link."}
                  </p>
                )}
                ...
                {magicLink && (
                  <div className="space-y-2 rounded-lg border bg-muted/30 p-3">
                    <p className="text-xs text-muted-foreground">
                      Live magic link generated and logged.
                    </p>
                    <div className="flex flex-wrap items-center gap-2">
                      <Input
                        value={magicLink}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-destructive/80">
                      A critical red flag triggered automatic session
                      termination.
                    </p>
                  </div>
                </div>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Conversation and candidate statements in chronological order.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-3">
                {transcript.length === 0 && (
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                ...
                {actionLog.map((entry, index) => (
                  <div
                    key={`${entry.time}-${index}`}
                    className="grid grid-cols-[1fr,1fr,auto] items-center gap-2 text-sm"
                  >
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-base">
                  Hiring Manager Activity Map
                </CardTitle>
                <CardDescription>
                  Heatmap view of session event intensity.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <ContributionGraph
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Preview the uploaded resume PDF for this candidate. No
                  hardcoded resume content is shown.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => void fetchResumePreview(session.id)}
                    >
                      Refresh
                    </Button>
                    {resolvedResumeUrl && (
                ...
                <div className="mt-3 overflow-hidden rounded-lg border">
                  {resolvedResumeUrl ? (
                    ...
                    <div className="flex h-[240px] items-center justify-center bg-muted/20 p-6 text-sm text-muted-foreground">
                      Resume PDF not available yet. Upload a resume to the
                      `resumes` storage bucket and set
                      `candidates.resume_storage_path`, or attach a PDF artifact
                      to this session.
                    </div>
                  )}
                </div>
          ...
          <aside className="space-y-6">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-amber-700 dark:text-amber-400 mt-1">
                        Scores generate automatically when rounds complete. Or
                        trigger manually:
                      </p>
                    </div>
                    <Button
                      ...
                      onClick={async () => {
                        try {
                          ...
                          alert(
                            "Scoring triggered! Check Gate Panel in a few seconds.",
                          );
                        } catch (err) {
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-base">
                  Role Widget Configuration
                </CardTitle>
                <CardDescription>
                  Hiring manager configurable flows for candidate-side role
                  widgets. Saved config streams live to candidate view.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-3">
                <div className="space-y-2">
                  ...
                  <Select
                    value={widgetRoleFamily}
                    onValueChange={setWidgetRoleFamily}
                  >
                    <SelectTrigger>
                ...
                <div className="flex flex-wrap gap-2">
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={loadRoleWidgetTemplate}
                  >
                    Load Recommended Template
                  </Button>
                  <Button
                    size="sm"
                    onClick={saveRoleWidgetConfig}
                    disabled={widgetConfigState === "saving"}
                  >
                    {widgetConfigState === "saving" ? (
                      <Loader2 className="h-4 w-4 animate-spin" />
                    ) : null}
                ...
                {widgetConfigState === "saved" && (
                  <p className="text-xs text-emerald-600">
                    Role widget configuration saved.
                  </p>
                )}
                {widgetConfigError && (
                    ...
                    <div className="grid gap-3 md:grid-cols-2">
                      <div className="space-y-2">
                        ...
                        <div className="flex items-center gap-2">
                          <Select
                            value={curveball}
                            onValueChange={setCurveball}
                          >
                            <SelectTrigger className="h-8">
                              <SelectValue />
                            </SelectTrigger>
                            <SelectContent>
                              <SelectItem value="budget_cut">
                                Budget cut
                              </SelectItem>
                              <SelectItem value="security_concern">
                                Security concern
                              </SelectItem>
                              <SelectItem value="timeline_mismatch">
                                Timeline mismatch
                              </SelectItem>
                              <SelectItem value="competitor_pressure">
                                Competitor pressure
                              </SelectItem>
                            </SelectContent>
                          </Select>
                    ...
                    <div className="grid gap-3 md:grid-cols-2">
                      <div className="space-y-1">
                        ...
                        <Select value={flagType} onValueChange={setFlagType}>
                          ...
                          <SelectContent>
                            {Object.entries(RED_FLAG_TYPES).map(
                              ([key, val]) => (
                      ...
                      <div className="space-y-1">
                        ...
                        <Select
                          value={flagSeverity}
                          onValueChange={(v) =>
                            setFlagSeverity(v as "warning" | "critical")
                          }
                        >
                          <SelectTrigger className="h-8">
                            <SelectValue />
                    ...
                    <Button
                      ...
                      onClick={() => {
                        sendAction("flag_red_flag", {
                          ...
                          description:
                            flagDescription ||
                            RED_FLAG_TYPES[flagType as RedFlagTypeKey]
                              ?.description ||
                            "",
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Saved to session action logs when submitted.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-3">
                <Textarea
                ...
                <div className="rounded-lg border bg-muted/30 p-3 text-xs text-muted-foreground">
                  ...
                  All interviewer actions, notes, and magic-link events are
                  logged for audit and post-assessment review.
                </div>
              </CardContent>
            </Card>
                        ...
                        onKeyDown={(event) => {
                          if (event.key === "Enter") {
                            ...
                            setLocalFollowups((prev) => {
                              if (prev.some((item) => item.question === value))
                                return prev;
                              return [
                                ...prev,
                                {
                            ...
                            >
                              ...
                              <Button
                                ...
                                onClick={() => {
                                  ...
                                  setLocalFollowups((prev) => {
                                    if (
                                      prev.some((row) => row.question === item)
                                    )
                                      return prev;
                                    return [
                                      ...prev,
