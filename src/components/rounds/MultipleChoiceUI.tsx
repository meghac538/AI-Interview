...
function buildOptions(config: Record<string, any>): MCQOption[] {
  ...
  return raw
    .map((option, index) => {
      ...
      const value =
        option.value || option.answer || label || code || fallbackId;
...
export function MultipleChoiceUI({ round }: { round: Round }) {
  ...
  if (!options.length) {
    return (
      <div className="rounded-xl border bg-card px-4 py-3 text-sm text-muted-foreground">
        Multiple-choice options are missing for this round. Please notify the
        interviewer.
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-xl border bg-card px-4 py-3 text-sm text-muted-foreground">
        Select the best answer. You can change your selection before submitting
        the round.
      </div>
        ...
        {options.map((option, index) => {
          ...
          return (
            ...
            >
              ...
              {option.code && (
                <pre className="mt-3 overflow-auto rounded-lg bg-muted/50 p-3 text-xs">
      ...
      <div className="flex items-center justify-between rounded-xl border bg-card px-4 py-3 text-xs text-muted-foreground">
        <span>
          {selected ? `Selected: ${selected}` : "No option selected yet."}
        </span>
